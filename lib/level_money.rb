...
class CLI < Thor
  ...
  method_option :ignore_cc_payments, :type => :boolean, :default => false
  def report(*params)
    exclude_transactions_by_merchant = []
    if options[:ignore_donuts] == true
      exclude_transactions_by_merchant << "Krispy Kreme Donuts"
      exclude_transactions_by_merchant << "DUNKIN #336784"
    end
    ...
    report.add_transaction_filter = {:scope => :collection, :name => :credit_card_payments} if options[:ignore_cc_payments]
    exclude_transactions_by_merchant.each  do |f|
      report.add_transaction_filter( {:scope => :transaction, :field => :merchant, :matcher => f })
    end
    report.filter!
